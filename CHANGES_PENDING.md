...
**Context.** Full investigation completed 2026-06-10 (in the NFT *Inventory* chat — this section is the handoff to the explorer chat). The collection's rarity was reverse-engineered from `all_nfts_metadata.json` + the HashLips design and reconciled against BBL's live marketplace API (via HAR capture). Everything below is decided and data-backed; what remains is page wiring + the proposal.

**The two ranking systems (both have canonical data files in `defipatriot/nft-metadata`):**
| | Intended (design) | BBL (marketplace) |
|---|---|---|
| File | `adao-rarity-intended.json` ✅ committed | `adao-rarity-bbl.json` — produced by weekly GitHub Action in same repo |
| Method | **Object trait alone defines the grade** (40 objects ↔ 40 grades, 1:1; metadata `Rarity` attribute = that grade). Grade order follows HashLips **planned weights**, not realized counts: Phoenix Rising planned 12 → Grade 40 apex (ranks #1–25) even though 25 minted vs Saber's 6. Grades laid end-to-end apex-first → 1–10,000 `intended_rank`; within a grade tokens are equal-rarity (ordered by token id only to assign distinct numbers). Planet/Inhabitant ≈ flat 500 each (identity), Light/Weather = atmosphere — **none are rarity factors** | Generic inverse-frequency sum over **every** attribute — including the derived `Rarity` tier (Object double-counted) and `broken`/`rewards` status traits. Result: realized-count order (Saber #1–6 above Phoenix ~#48–66) and atmosphere leakage — the 6 `Weather: Lightning strike` tokens (common weapons) rank BBL #7–12 but intended #3,022–7,476 |
| Broken NFTs | All 10,000 ranked (grade is broken-agnostic) | **Mostly unranked (`rank:null`) but inconsistently** — e.g. #4395 broken yet ranked #5755. This is the bug Camron saw: low-to-high sort piles unranked broken at one end |
| Spot-check | #9068 (Phoenix) = grade 40, rank **24** | #9068 = rank **68** |

**BBL Action status:** `bbl-rarity.js` + `.github/workflows/bbl-rarity.yml` live in `nft-metadata`. Run #1 correctly hard-failed at the integrity gate (captured 9,342/10,000 — BBL's null-rank block has no stable sort key, so paginating it drops/dupes rows). Patched same day: unreturned tokens are filled as `bbl_rank:null` (by construction they're in the null block = unranked), sanity floor 8,500, provenance fields `captured_from_bbl`/`filled_unreturned`. **Pending: re-upload patched `bbl-rarity.js`, re-run workflow, confirm `adao-rarity-bbl.json` commits** (~8.9K ranked / ~1.09K unranked). Writes only when ranks actually changed → quiet weeks = no commit → file `built` date = "last time BBL ranks moved." Deliberately isolated: if BBL dies, delete 2 files + the toggle; nothing else affected.

**Explorer changes (Camron's spec, 2026-06-10) — stage on `test.html` first:**
1. **[ ]** Load both rarity files; join to records by `token_id`.
2. **[ ]** **Rank-system toggle** `BBL Rank / Intended Rank`, switching every rank shown. Display per system: Intended → `Rarity 40, Rank 24`-style; BBL → `Rarity 40, Rank 68` (keep the grade visible in both; the *rank* is what switches). BBL + broken → show "Unranked" when `bbl_rank:null` (faithful to BBL).
3. **[ ]** BBL-mode disclaimer line near the toggle: mirrored from BBL · last changed {file `built` date} · BBL leaves most broken NFTs unranked.
4. **[ ]** Display-option toggles — defaults ON: **Rank, Planet, Inhabitant, Object**; defaults OFF/hidden: **Weather, Light, Rarity** (the old `40/1`-style rarity-class line is retired from default view).
5. **[ ]** Filter dropdowns: still 4; **replace the Rarity dropdown with Rank** (filters by the 1–40 grade under the hood — a 10,000-option exact-rank dropdown is impractical; confirmed intent).
6. **[ ]** **Sort By** options: `Ranking, Rarity, ID` — default **Ranking High→Low** (i.e. best rank first, honoring the active toggle).
...
8. **[ ]** The explorer's internal sub-rank computation (rarityClass + Weather/Light tie-break, the source of the old `40/1` display) is superseded — ranks come from the canonical files only.

**Site page:** `rarity-explained.html` fully rewritten (Rev 2.0, in outputs 2026-06-10 — TOC box removed per review): HashLips + weights story, layer roles, designed grade ladder w/ planned-vs-actual table, intended-rank construction, the BBL weather example (real tokens/ranks), BBL-as-primary-marketplace acknowledgment, references (HashLips repo, official Notion rarity doc, both JSON files, metadata). Replaces the old page at site root; dashboard + explorer footers both link here.

**DAO proposal (to draft, separate task):** adopt Intended as the collection's official grading; ask Atrium to grade by it (file is open + verifiable: `raw.githubusercontent.com/defipatriot/nft-metadata/main/adao-rarity-intended.json`); evidence = design intent (Object-only, planned weights, Phoenix apex) + BBL divergences (weather leakage, apex inversion, inconsistent broken handling) + the explorer toggle as the both-worlds bridge.
