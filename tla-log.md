...
#### Replace explain mode with per-panel expandable descriptions; header tidy-ups
...
#### Explain mode — plain-language on-ramp for newcomers
...
#### Member positions: earnings, APR/share, amplified tag + pool-type note everywhere
...
#### Pool Health: group by bucket, top-3 each + expandable
...
#### Liquidity Growth: recalibrate verdict + add size/migration context
...
#### New: '''Is TLA Liquidity Growing?''' panel + Threshold reorg + per-token flows
...
#### Composition: pool-type aware (concentrated vs xyk vs stable)
...
#### Revert the LST priceSuspect guard — arbLUNA price was correct
...
#### Threshold Watch: split into the two intended warnings
...
#### Composition: guard against bad LST price feeds (the 62/38 bug)
...
#### Pool Health: LP composition strip + IL sensitivity
...
#### Remove per-card STALE-SOON badge (footer health check only)
...
#### Pool Health & Exit Risk: made legible
...
#### Fix: locked-in active/inactive split must reflect boundary status
...
#### Fix: demoted variants inflating the inactive group in locked-in view
...
#### Data audit vs Eris/Votion + two central variant safeguards
...
#### Waterfall: clamp duplicated Votion onto dead gauge variants
...
#### Waterfall: fix locked-in double-count + amber inactive
...
#### Waterfall: real locked-in vs planned baseline
...
#### Waterfall: planned-delta label placement
...
#### Waterfall: color scheme + planned-epoch deltas
...
#### Waterfall: fixed label gutter + aligned plot area
...
#### Vote Breakdown waterfall: cleaner labels + grouped inactive
...
#### Overview tiles: move toggle inline for uniform tile heights
...
#### Popup: bribe token amounts + Votion zero-sum note
...
#### Ranking popup redesigned: prior -> current -> change table
...
#### Overview tiles: APR rank arrows restored + Eris reconciliation
...
#### Overview tiles: TLA-staked field fix + honest APR comparisons
...
#### Overview tiles: clean uniform rows + drivers moved to popup
...
#### Overview tiles: fixes + cleaner drivers
...
#### Overview leaderboard tiles enriched (new)
...
#### Newcomer protection: per-pool risk flags + APR caveat (new)
...
#### Member reward/bribe tiles relabeled + token breakdowns (new)
...
#### Member selector now activity-filtered
